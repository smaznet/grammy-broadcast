...
function getMiddleware(options) {
  var _a;
  const broadcastMiddleware = ((_a = options.sudoUsers) == null ? void 0 : _a.length) ? new Composer().filter((ctx) => {
    var _a2;
...
Messages Count ${currentIds.length}`, {
  ...
  broadcastMiddleware.callbackQuery(/brd:stop:(\w+)/, async (ctx) => {
    return ctx.editMessageReplyMarkup({
      reply_markup: new InlineKeyboard().text("Sure?").row().text("Yes", "brd:stop_confirm:" + ctx.match[1]).text("No", `brd:stop_cancel:${ctx.match[1]}`)
    });
  });
